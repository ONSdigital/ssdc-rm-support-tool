...
class SurveyCaseSearch extends Component {
  ...
  onSearch = async () => {
    ...
    if (this.state.selectedRefusal !== NOT_REFUSED) {
      searchUrl += '&refusal=' + this.state.selectedRefusal
    }
  ...
  onFilterRefusal = (event) => {
    this.setState({
      selectedRefusal: event.target.value
    })
  }
        ...
        <div style={{ margin: 10 }}>
          ...
          <FormControl style={{ minWidth: 200 }}>
            ...
            <Select
              ...
              value={this.state.selectedRefusal}>
              {refusalMenuItems}
            </Select>
          </FormControl>
        </div>
