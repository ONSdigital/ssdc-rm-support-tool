  ...
  public String[] getExpectedColumns(JobType jobType, CollectionExercise collectionExercise) {
    return switch (jobType) {
      case SAMPLE -> {
        ...
        yield Arrays.stream(columnValidators)
            .map(columnValidator -> columnValidator.getColumnName())
            .collect(Collectors.toList())
            .toArray(new String[0]);
      }
      case BULK_REFUSAL -> new String[] {"caseId", "refusalType"};
      case BULK_INVALID -> new String[] {"caseId", "reason"};
      case BULK_UPDATE_SAMPLE, BULK_UPDATE_SAMPLE_SENSITIVE ->
          new String[] {"caseId", "fieldToUpdate", "newValue"};
    };
  }
